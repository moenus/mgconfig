...
from mgconfig.sec_store_crypt import bytes_to_b64str, b64str_to_bytes
...
def test_new_header():
    ...
    with patch('time.time', return_value=current_time):
        header = SecurityHeader.create_new(master_key_hash)
...
def test_create_header_from_dict(sample_header_dict):
    ...
    header = SecurityHeader.prepare(sample_header_dict)
...
def test_create_header_missing_field():
    ...
    with pytest.raises(ValueError, match="header missing 'salt_b64'"):
        SecurityHeader.prepare(incomplete_dict)
