...
import mgconfig.keystore as keystore
...
def test_keystores_registry():
    ks_env = keystore.KeyStoreEnv()
    keystore.KeyStores._ks_dict.clear()
    keystore.KeyStores.add(ks_env)
    assert keystore.KeyStores.contains("env")
    assert keystore.KeyStores.get("env") is ks_env

    with pytest.raises(ValueError):
        keystore.KeyStores.add(ks_env)  # duplicate
