...
def mock_defaults(monkeypatch):
    ...
    monkeypatch.setattr("mgconfig.config_defs.DefaultFunctions", lambda: mock_funcs)

    mock_vals = MagicMock()
    mock_vals.dict = {"prefix_name": "val_default"}
    mock_vals.get.side_effect = lambda key: mock_vals.dict.get(key)
    monkeypatch.setattr("mgconfig.config_defs.DefaultValues", lambda: mock_vals)
...
def test_parse_config_defs_success(tmp_path, mock_defaults):
    ...
    assert "prefix_name" in cfg.cfg_defs
    cfg_def = cfg.cfg_defs["prefix_name"]
...
def test_invalid_identifier(monkeypatch):
    monkeypatch.setattr("mgconfig.config_defs.ConfigTypes._config_types", ["string"])
    with pytest.raises(ValueError) as e:
...
def test_default_value_type_mismatch(monkeypatch):
    monkeypatch.setattr("mgconfig.config_defs.ConfigTypes._config_types", ["int"])
    monkeypatch.setattr("mgconfig.config_defs.ConfigTypes.parse_value", lambda v, t: (False, None))
    with pytest.raises(ValueError) as e:
